...
import { CONTEXT_WINDOW_EXCEEDED_CODE, ToolCallId, ToolCallId as CallId, EMPTY_RESPONSE_CODE, LlmAdapter, LlmError, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId, RetryPolicySchema, contentHasImage, isContextWindowExceededError, isQuotaExceededError, resolveRetryPolicy, userAgent } from "@deepseek-ai/dsh-llm";
...
*/
function kiroServiceRegion(connection, token) {
	const profileArn = connection.profileArn ?? token.profileArn;
	if (profileArn !== void 0) return profileRegion(profileArn);
	if (connection.region !== void 0) return connection.region;
...
var KiroAdapter = class extends LlmAdapter {
	...
	resolvedModel(provider, model, connection, catalog) {
	...
	async *streamWithConnection(options, connection, catalog) {
		const env_1 = {
			stack: [],
			error: void 0,
			hasError: false
		...
		try {
			...
			const iterator = this.request(options, watchdog.signal, connection, catalog, () => {
				watchdog.pulse();
	...
	async *request(options, signal, connection, catalog, onActivity) {
		const token = await this.config.resolveToken(connection, signal);
		const profileArn = connection.profileArn ?? token.profileArn;
		const url = kiroRequestEndpoint(token, kiroServiceRegion(connection, token));
...
async function discoverKiroProfileArn(connection, token, signal, request$2 = postJsonWithHeaders) {
	...
	const candidates = [...new Set([
		kiroServiceRegion(connection, token),
		"us-east-1",
		"eu-central-1"
	...
	for (const candidate of candidates) {
		...
		for (const attempt of attempts) {
			let response;
			try {
				response = await request$2(attempt.url, { maxResults: 50 }, attempt.headers, connection.proxyUrl, signal);
			} catch (error) {
		...
		if (reachedEndpoint) return void 0;
	}
	if (firstTransportError !== void 0) throw firstTransportError;
}
...
var KiroModelDiscovery = class {
	...
	*/
	async list(connection, signal, force = false) {
		...
		if (profileArn === void 0) try {
			profileArn = await this.discoverProfile(connection, token, serviceRegion, signal);
		} catch (error) {
			if (signal.aborted) throw error;
		}
		const profileRegion$1 = profileArn?.split(":")[3];
		const region = profileRegion$1 !== void 0 && /^[a-z]{2}(?:-[a-z0-9]+)+-[0-9]+$/u.test(profileRegion$1) ? profileRegion$1 : serviceRegion;
...
//#endregion
export { BUILDER_START_URL, Config, DEFAULT_CONTEXT_WINDOW, DEFAULT_REGION, DEFAULT_STREAM_IDLE_TIMEOUT_MS, FileModelSettingsStore, KiroAdapter, KiroModelDiscovery, KiroUsageService, apply, assertKiroProfileArn, assertKiroRegion, assertMicrosoftTokenEndpoint, buildModelRequestFields, clearTokenCache, compareKiroModels, credentialDirectory, credentialSummary, deleteDeviceCredentials, discoverKiroProfileArn, getJson, httpErrorCode, importApiKey, importExternalIdp, importRefreshToken, inject, kiroAuthMethod, kiroCredentialDirectory, kiroServiceRegion, modelPageToken, modelSelection, modelSettingsPath, modelSupportsThinking, name, normalizeExternalIdpCredentials, parseAvailableModels, parseEffortSchema, parseKiroUsage, parseMaxTokensBounds, parseProxyUrl, pollDeviceLogin, pollSocialDeviceLogin, postForm, postJson, postJsonWithHeaders, profileRegion, resolveAdapterOptions, resolveRefreshTokenOrigin, resolveToken, resolveTokenFromDirectories, saveDeviceCredentials, saveManagedCredentials, startDeviceLogin, startSocialDeviceLogin };