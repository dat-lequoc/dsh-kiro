...
import { CONTEXT_WINDOW_EXCEEDED_CODE, CallId, EMPTY_RESPONSE_CODE, LlmAdapter, LlmError, ProviderRequestId, ReasoningEffortId, RetryPolicySchema, contentHasImage, isContextWindowExceededError, resolveRetryPolicy, userAgent } from "@deepseek-ai/dsh-llm";
...
*/
function serializeRequest(options, defaults, conversationId, profileArn, nativeEffort, limits) {
...
var KiroAdapter = class extends LlmAdapter {
	...
	async *request(options, signal, connection, onActivity) {
		...
		const body = JSON.stringify(serializeRequest(options, connection.defaults, conversationIdFor(options.sessionId), profileArn, nativeEffort, selected?.maxTokensBounds === void 0 ? void 0 : { maxTokensBounds: selected.maxTokensBounds }));
...
//#endregion
export { BUILDER_START_URL, Config, DEFAULT_CONTEXT_WINDOW, DEFAULT_REGION, DEFAULT_STREAM_IDLE_TIMEOUT_MS, FileModelSettingsStore, KiroAdapter, KiroModelDiscovery, KiroUsageService, apply, assertKiroProfileArn, assertKiroRegion, assertMicrosoftTokenEndpoint, buildModelRequestFields, clearTokenCache, compareKiroModels, credentialDirectory, credentialSummary, deleteDeviceCredentials, discoverKiroProfileArn, getJson, httpErrorCode, importApiKey, importExternalIdp, importRefreshToken, inject, kiroAuthMethod, kiroCredentialDirectory, modelPageToken, modelSelection, modelSettingsPath, modelSupportsThinking, name, normalizeExternalIdpCredentials, parseAvailableModels, parseEffortSchema, parseKiroUsage, parseMaxTokensBounds, parseProxyUrl, pollDeviceLogin, pollSocialDeviceLogin, postForm, postJson, postJsonWithHeaders, profileRegion, resolveAdapterOptions, resolveRefreshTokenOrigin, resolveToken, resolveTokenFromDirectories, saveDeviceCredentials, saveManagedCredentials, startDeviceLogin, startSocialDeviceLogin };