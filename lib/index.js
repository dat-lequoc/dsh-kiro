...
*/
function serializeRequest(options, defaults, conversationId, profileArn, nativeEffort) {
...
async function* translate(frames) {
	...
	for await (const frame of frames) {
		...
		switch (frame.headers[":event-type"]) {
			...
			case "metadataEvent":
				usage = tokenUsageOf(parsePayload(frame).tokenUsage) ?? usage;
				break;
			default: break;
		}
	}
	...
	const reason = toolBlocks.size > 0 ? { kind: "tool-calls" } : order.length > 0 ? { kind: "stop" } : {
		kind: "error",
		failure: {
...
var KiroAdapter = class extends LlmAdapter {
	...
	async *request(options, signal, connection, onActivity) {
		...
		const body = JSON.stringify(serializeRequest(options, connection.defaults, randomUUID(), profileArn, nativeEffort));
...
function registerWebApi(ctx, dependencies) {
	...
	const importCredential = async (body) => {
		...
		} else if (method === "api-key") credentials = await importApiKey(requiredText(body.apiKey, "Kiro API key"), optionalText(body.region) ?? connection.region, (url, headers$1, requestSignal) => getJson(url, headers$1, connection.proxyUrl, requestSignal), signal);
...
function row(id, label, used, limit, resetAt, kind) {
	...
	return {
		...
		limit,
		remaining,
		usedPercent: limit > 0 ? percentage(used / limit * 100) : 0,
...
function parseKiroUsage(value, now = Date.now()) {
	...
	for (const raw of breakdowns) {
		...
		rows.push(row(resourceType.toLowerCase(), label, numeric(item.currentUsageWithPrecision, item.currentUsage), numeric(item.usageLimitWithPrecision, item.usageLimit), resetAt, "subscription"));
		const trial = record(item.freeTrialInfo);
		const trialExpiry = timestamp(trial?.freeTrialExpiry);
		if (trial !== void 0 && trial.freeTrialStatus !== "EXPIRED" && (numeric(trial.usageLimitWithPrecision, trial.usageLimit) > 0 || trialExpiry !== void 0 && Date.parse(trialExpiry) > now)) rows.push(row(`${resourceType.toLowerCase()}-welcome-bonus`, "Welcome bonus", numeric(trial.currentUsageWithPrecision, trial.currentUsage), numeric(trial.usageLimitWithPrecision, trial.usageLimit), trialExpiry ?? resetAt, "bonus"));
	}
...
var KiroUsageService = class {
	...
	async get(connection, signal, force = false) {
		...
		const attempts = [
			() => this.getRequest(`https://codewhisperer.${region}.amazonaws.com/getUsageLimits?${codeWhispererQuery.toString()}`, commonHeaders, connection.proxyUrl, signal),
			() => this.postRequest(`https://codewhisperer.${region}.amazonaws.com`, {
				origin: "AI_EDITOR",
				resourceType: "AGENTIC_REQUEST",
				...profileArn === void 0 ? {} : { profileArn }
			}, {
			...
			() => this.getRequest(`https://q.${region}.amazonaws.com/getUsageLimits?${qQuery.toString()}`, commonHeaders, connection.proxyUrl, signal)
		];
		let lastStatus = 0;
		let lastError;
...
//#endregion
export { BUILDER_START_URL, Config, DEFAULT_CONTEXT_WINDOW, DEFAULT_REGION, DEFAULT_STREAM_IDLE_TIMEOUT_MS, FileModelSettingsStore, KiroAdapter, KiroModelDiscovery, KiroUsageService, apply, assertKiroProfileArn, assertKiroRegion, assertMicrosoftTokenEndpoint, clearTokenCache, compareKiroModels, credentialDirectory, credentialSummary, deleteDeviceCredentials, discoverKiroProfileArn, getJson, httpErrorCode, importApiKey, importExternalIdp, importRefreshToken, inject, kiroCredentialDirectory, modelSelection, modelSettingsPath, modelSupportsThinking, name, normalizeExternalIdpCredentials, parseAvailableModels, parseEffortSchema, parseKiroUsage, parseProxyUrl, pollDeviceLogin, pollSocialDeviceLogin, postForm, postJson, postJsonWithHeaders, profileRegion, resolveAdapterOptions, resolveToken, resolveTokenFromDirectories, saveDeviceCredentials, saveManagedCredentials, startDeviceLogin, startSocialDeviceLogin };