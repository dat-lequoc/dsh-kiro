	...
	for (const message of messages) {
		...
		if (message.role === "assistant") {
			...
			if (last$1?.role === "assistant") {
				last$1.text = [last$1.text, text].filter((part) => part.length > 0).join("\n\n");
...
async function resolveToken(options) {
	...
	const token = await readJsonFile(join(directory, TOKEN_FILE$1), "token file");
	let region;
	try {
...
var KiroModelDiscovery = class {
	...
	async discoverProfile(connection, token, region, signal) {
		...
		for (const candidate of candidates) {
			const response = await this.profileRequestJson(`${this.endpoint(candidate)}/ListAvailableProfiles`, { maxResults: 50 }, this.headers(token), connection.proxyUrl, signal);
			if (response.status !== 200) continue;
			const profiles = record$1(response.body)?.profiles;
			if (!Array.isArray(profiles)) continue;
			for (const raw of profiles) {
				const arn = record$1(raw)?.arn;
				if (typeof arn === "string" && /^arn:aws:codewhisperer:[a-z]{2}(?:-[a-z0-9]+)+-[0-9]+:[0-9]{12}:profile\/[A-Za-z0-9+=,.@_-]+$/u.test(arn.trim())) return arn.trim();
			}
		}
	}
	/**
	...
	*/
	async list(connection, signal, force = false) {
		...
		const profileArn = connection.profileArn ?? await this.discoverProfile(connection, token, authRegion, signal);
		const profileRegion = profileArn?.split(":")[3];
		const region = profileRegion !== void 0 && /^[a-z]{2}(?:-[a-z0-9]+)+-[0-9]+$/u.test(profileRegion) ? profileRegion : authRegion;
...
*/
async function startDeviceLogin(region, requestJson, signal) {
	...
	if (registration.status !== 200) throw new Error(`Kiro client registration failed: ${providerError(registration.body, `HTTP ${registration.status}`)}`);
	...
	if (verificationUrl.protocol !== "https:" || !(verificationUrl.hostname.endsWith(".amazonaws.com") || verificationUrl.hostname.endsWith(".awsapps.com"))) throw new Error(`Kiro returned an unsafe verification URL host: ${verificationUrl.hostname}`);
...
*/
async function saveDeviceCredentials(directory, credentials) {
...
async function credentialSummary(directory) {
	...
	try {
		...
		if (error.code === "ENOENT") return { authenticated: false };
...
function safeError(error) {
	return (error instanceof Error ? error.message : String(error)).replace(/Bearer\s+\S+/giu, "Bearer [redacted]").replace(/(?:access|refresh|client)[_-]?(?:token|secret)\s*[:=]\s*\S+/giu, "[redacted credential]");
}
...
function registerWebApi(ctx, dependencies) {
	...
	const status = async () => {
		...
		const external = managed.authenticated ? { authenticated: false } : await credentialSummary(kiroCredentialDirectory());
	...
	const beginLogin = async () => {
		...
		const requestJson = (url, body, signal) => postJson(url, body, connection.proxyUrl, signal);
...
//#endregion
export { Config, DEFAULT_CONTEXT_WINDOW, DEFAULT_REGION, DEFAULT_STREAM_IDLE_TIMEOUT_MS, KiroAdapter, KiroModelDiscovery, apply, assertKiroRegion, clearTokenCache, credentialDirectory, credentialSummary, deleteDeviceCredentials, getJson, httpErrorCode, inject, kiroCredentialDirectory, modelSupportsThinking, name, parseAvailableModels, parseProxyUrl, pollDeviceLogin, postJson, postJsonWithHeaders, resolveAdapterOptions, resolveToken, resolveTokenFromDirectories, saveDeviceCredentials, startDeviceLogin };