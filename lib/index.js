...
async function startDeviceLogin(region, requestJson, signal, options = {}) {
	...
	const authMethod = options.authMethod === "idc" || options.authMethod === "google" || options.authMethod === "github" ? options.authMethod : "builder-id";
...
function registerWebApi(ctx, dependencies) {
	...
	const beginDevice = async (body) => {
		...
		if (body.method !== "builder-id" && body.method !== "idc" && body.method !== "google" && body.method !== "github") throw new Error("Unsupported Kiro device login method");
...
//#endregion
export { BUILDER_START_URL, Config, DEFAULT_CONTEXT_WINDOW, DEFAULT_REGION, DEFAULT_STREAM_IDLE_TIMEOUT_MS, KiroAdapter, KiroModelDiscovery, apply, assertKiroProfileArn, assertKiroRegion, assertMicrosoftTokenEndpoint, clearTokenCache, credentialDirectory, credentialSummary, deleteDeviceCredentials, discoverKiroProfileArn, getJson, httpErrorCode, importApiKey, importExternalIdp, importRefreshToken, inject, kiroCredentialDirectory, modelSupportsThinking, name, normalizeExternalIdpCredentials, parseAvailableModels, parseProxyUrl, pollDeviceLogin, postForm, postJson, postJsonWithHeaders, profileRegion, resolveAdapterOptions, resolveToken, resolveTokenFromDirectories, saveDeviceCredentials, saveManagedCredentials, startDeviceLogin };